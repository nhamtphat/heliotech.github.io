...
Mở file migration vừa tạo trong thư mục `database/migrations/xxx_create_messages_table.php`. Nội dung của bảng `massages` gồm các cột:
...
Input `wire:model="message"` giống như Vue hay Angular, nó cho phép bạn tự động gán biến `$this->massage` vào input đó. Input thay đổi thì biến này sẽ thay đổi theo.
...
Phần 2 đang được thực hiện, để nhận được thông báo về các bài viết mới trên Heliotech:
