...
Tiếp theo bạn cần tạo một file cấu hình biến môi trường, hãy copy file `.env.example` ra một file mới và đặt tên là `.env.ci`. Trong file này bạn chỉ cần chú ý đến chỗ cài đặt các biến CSDL, đối với các bạn chạy [MySQL](https://heliotech.me/blog/connect-laravel-with-mysql) thì hãy đổi lại như sau:
...
![Untitled](/posts/setup-github-action-for-unittest/1.png)
...
SQLite rất cấu hình rát đơn giản, bạn hãy xoá luôn `services` bên trong file cấu hình workflow, và sửa nội dung `.env.ci` như sau:
